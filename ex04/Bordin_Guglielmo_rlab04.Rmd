...
to be 0 km and 5 km, respectively. We sample the positions by generating
...
Since we’re taking products to calculate the overall likelihood of the
data, it’s more convenient to take the logarithm first and later
exponentiate the log-posterior.
```{r}
log_lhood <- Reduce(
    \(x, y) x - log(1 + ((y - alpha) / beta)^2), data, init = 0
)
```